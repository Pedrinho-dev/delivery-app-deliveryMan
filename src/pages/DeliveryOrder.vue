...
async function loadMapRoute() {
  ...
  let clientLoc = parseCoords(order.value.clientLoc);
  if (!clientLoc && order.value.clientLoc) {
    clientLoc = await getCoordinatesFromAddress(order.value.clientLoc);
  }

  let destLoc = parseCoords(order.value.destinyLoc);
  if (!destLoc && order.value.destinyLoc) {
    destLoc = await getCoordinatesFromAddress(order.value.destinyLoc);
  }
